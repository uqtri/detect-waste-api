...
model_path = hf_hub_download(
    repo_id="kendrickfff/my_resnet50_garbage_classification",
    filename="resnet50_garbage_classification.pth"
)

# Load model
classifier = models.resnet50(weights=None)
classifier.fc = torch.nn.Linear(classifier.fc.in_features, 12)
...
def save_frames(args, img_size, num_iter=45913):
    ...
    while(cap.isOpened()):
        ...
        for p, (xmin, ymin, xmax, ymax) in zip(
            ...
            pred = torch.topk(outputs, k=1).indices.squeeze(0).tolist()[0]
            p[0] = torch.softmax(outputs, dim=1)[0, int(pred)].item()
            p[1] = pred + 1
            if p[0] >= args.cls_th:
    ...
    if args.video:
        ...
        for f in dir_list:
            ...
            for p, (xmin, ymin, xmax, ymax) in zip(
                ...
                img = cls_transform(crop).unsqueeze(0)
                ...
                print(f"Predicted class: {args.classes[pred]}, Confidence: {conf}")
                # plot_results/get_output expect class id to be 1-based.
                p[1] = pred + 1
                p[0] = conf
